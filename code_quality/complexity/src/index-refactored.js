...
function guessFish(animalData) {
  return animalData.isPredator ? 'shark' : 'clown fish';
}

function guessWaterMammal(animalData) {
  return animalData.isBig ? 'whale' : 'dolphin';
}

function guessAnimalWithLegs(animalData) {
  return animalData.doesBark ? 'dog' : 'cat';
}

function guessAnimalWithougLegs(animalData) {
  return animalData.hasShell ? 'snail' : 'snake';
}
